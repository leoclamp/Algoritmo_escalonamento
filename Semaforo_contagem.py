...
def download_part(url, part_num, start, end, sem):
    sem.acquire()  # Semáforo de contagem controla o acesso
    try:
...
def download_with_counting_semaphore(url, num_parts):
    response = requests.head(url)
    file_size = int(response.headers.get('Content-Length', 0))
    part_size = file_size // num_parts
    futures = []
    sem = Semaphore(2)  # Semáforo de contagem (permite 2 acessos simultâneos)
...
download_with_counting_semaphore(url, num_parts)
end_time = time.time()
print(f"Tempo total com semáforo de contagem: {end_time - start_time:.2f} segundos")
